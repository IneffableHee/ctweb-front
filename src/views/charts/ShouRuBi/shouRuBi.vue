<template>
	<div class="content-box">
		...
		<!-- 规则来源 -->
		<div style="width: 100%;">
			<el-dialog title="取数规则:" class="mybox" :append-to-body="true" :visible.sync="yuJingDialogVisible" @update:visible="newVisible => yuJingDialogVisible = newVisible">
	...
	export default {
		...
		methods: {
			open4() {
				this.$notify({
					title: '警告',
					message: '成本收入比已高于35%，'+this.yuJingData+'请到相关页面查看详细信息！',
					type: 'warning',
					duration: 9000
				})
			},
			...
			initEchart() {
				...
				getNHLiRunShouRu(params).then((res) => {
					...
					_this.dataChengBen = res.data.data.成本收入比;
					_this.dataZhongJian = res.data.data.中间业务收入比;
					_this.RuleData = res.data.data.dataRules;
					for (var i = 0; i < _this.dataChengBen.length; i++) {
						if (_this.dataChengBen[i] >= 0.35) {

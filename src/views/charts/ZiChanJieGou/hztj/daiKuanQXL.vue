		...
		methods: {
			initEchart() { 
				...
				getBuLiangDaiKuanHZTJ(params).then((res) => {
					...
					_this.dataNomalDaiKuan = res.data.data.期初正常贷款在期末的存量;
					_this.dataBuLiang = res.data.data.正常类迁徙为不良的金额;
					_this.dataZB = res.data.data.贷款迁徙率;
					...
					let  option = {
						...
						legend: {
							top: '20',
							// bottom:20,
							data: ['期初正常贷款在期末的存量', '正常类迁徙为不良的金额', '贷款迁徙率'] 
						},
						toolbox: {
							show: true,
							// 查看预警条件
